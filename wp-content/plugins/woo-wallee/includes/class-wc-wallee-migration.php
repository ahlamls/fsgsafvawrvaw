...
class WC_Wallee_Migration {
	...
	private static function check_requirements() {
		...
		if ( version_compare( PHP_VERSION, WC_WALLEE_REQUIRED_PHP_VERSION, '<' ) ) {
			$errors[] = sprintf( __( "PHP %1\$s+ is required. (You're running version %2\$s)", 'woo-wallee' ), WC_WALLEE_REQUIRED_PHP_VERSION, PHP_VERSION );
		}
		if ( version_compare( $wp_version, WC_WALLEE_REQUIRED_WP_VERSION, '<' ) ) {
			$errors[] = sprintf( __( "Wordpress %1\$s+ is required. (You're running version %2\$s)", 'woo-wallee' ), WC_WALLEE_REQUIRED_WP_VERSION, $wp_version );
		...
		if ( ! is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
			...
			if ( version_compare( $woocommerce_data['Version'], WC_WALLEE_REQUIRED_WC_VERSION, '<' ) ) {
				$errors[] = sprintf( __( "Woocommerce %1\$s+ is required. (You're running version %2\$s)", 'woo-wallee' ), WC_WALLEE_REQUIRED_WC_VERSION, $woocommerce_data['Version'] );
			}
		}
	...
	public static function wpmu_drop_tables( $tables ) {
		...
		$tables[] = $wpdb->prefix . 'wc_wallee_method_config';
		$tables[] = $wpdb->prefix . 'wc_wallee_transaction_info';
		$tables[] = $wpdb->prefix . 'wc_wallee_token_info';
		$tables[] = $wpdb->prefix . 'wc_wallee_completion_job';
		$tables[] = $wpdb->prefix . 'wc_wallee_void_job';
		$tables[] = $wpdb->prefix . 'wc_wallee_refund_job';
	...
	public static function plugin_row_meta( $links, $file ) {
		if ( WC_WALLEE_PLUGIN_BASENAME === $file ) {
			$row_meta = array(
				'docs' => '<a href="@WalleeDocPath(docs/en/documentation.html)" aria-label="' . esc_attr__( 'View Documentation', 'woo-wallee' ) . '">' . esc_html__( 'Documentation', 'woo-wallee' ) . '</a>',
			);
	...
	public static function update_1_0_0_initialize() {
		...
		$result = $wpdb->query(
			"CREATE TABLE IF NOT EXISTS {$wpdb->prefix}wc_wallee_method_config(
		...
		$result = $wpdb->query(
			"CREATE TABLE IF NOT EXISTS {$wpdb->prefix}wc_wallee_transaction_info(
				`id` int(10) unsigned NOT NULL AUTO_INCREMENT,
		...
		$result = $wpdb->query(
			"CREATE TABLE IF NOT EXISTS {$wpdb->prefix}wc_wallee_token_info(
				`id` int(10) unsigned NOT NULL AUTO_INCREMENT,
		...
		$result = $wpdb->query(
			"CREATE TABLE IF NOT EXISTS {$wpdb->prefix}wc_wallee_completion_job(
				`id` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`state` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
		...
		$result = $wpdb->query(
			"CREATE TABLE IF NOT EXISTS {$wpdb->prefix}wc_wallee_void_job(
				`id` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`state` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
		...
		$result = $wpdb->query(
			"CREATE TABLE IF NOT EXISTS {$wpdb->prefix}wc_wallee_refund_job(
				`id` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`state` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
	...
	public static function update_1_0_1_image_url() {
		global $wpdb;
		$result = $wpdb->query(
			"ALTER TABLE `{$wpdb->prefix}wc_wallee_method_config` CHANGE `image` `image` VARCHAR(2047) CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL;"
		);
		if ( false === $result ) {
			throw new Exception( $wpdb->last_error );
		}
		...
		$result = $wpdb->query(
			"ALTER TABLE `{$wpdb->prefix}wc_wallee_transaction_info` CHANGE `image` `image` VARCHAR(2047) CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL;"
		);
		if ( false === $result ) {
			throw new Exception( $wpdb->last_error );
		}
	...
	public static function update_1_0_2_order_allow_null() {
		global $wpdb;
		$result = $wpdb->query(
			"ALTER TABLE `{$wpdb->prefix}wc_wallee_transaction_info` CHANGE `order_id` `order_id` int(10) unsigned NULL DEFAULT NULL;"
		);
		if ( false === $result ) {
			throw new Exception( $wpdb->last_error );
		}

		$result = $wpdb->query( "SHOW COLUMNS FROM `{$wpdb->prefix}wc_wallee_transaction_info` LIKE 'order_mapping_id'" );
		if ( 0 == $result ) {
			$result = $wpdb->query(
				"ALTER TABLE `{$wpdb->prefix}wc_wallee_transaction_info` ADD `order_mapping_id` int(10) unsigned NULL AFTER order_id;"
			);
			if ( false === $result ) {
				throw new Exception( $wpdb->last_error );
			}
		...
		$result = $wpdb->query(
			"ALTER TABLE `{$wpdb->prefix}wc_wallee_token_info` CHANGE `customer_id` `customer_id` int(10) unsigned NULL DEFAULT NULL;"
		);
	...
	public static function update_1_0_3_image_domain() {
		...
		$result = $wpdb->query( "SHOW COLUMNS FROM `{$wpdb->prefix}wc_wallee_method_config` LIKE 'image_base'" );
		if ( 0 == $result ) {
			$result = $wpdb->query(
				"ALTER TABLE `{$wpdb->prefix}wc_wallee_method_config` ADD COLUMN `image_base` VARCHAR(2047) CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL AFTER image;"
			);
			if ( false === $result ) {
				throw new Exception( $wpdb->last_error );
			}
		...
		$result = $wpdb->query( "SHOW COLUMNS FROM `{$wpdb->prefix}wc_wallee_transaction_info` LIKE 'image_base'" );
		if ( 0 == $result ) {
			$result = $wpdb->query(
				"ALTER TABLE `{$wpdb->prefix}wc_wallee_transaction_info` ADD COLUMN `image_base` VARCHAR(2047) CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL AFTER image;"
			);
			if ( false === $result ) {
				throw new Exception( $wpdb->last_error );
			}
	...
	public static function update_1_0_4_failure_msg_and_attribute() {
		...
		$result = $wpdb->query( "SHOW COLUMNS FROM `{$wpdb->prefix}wc_wallee_transaction_info` LIKE 'user_failure_message'" );
		if ( 0 == $result ) {
			$result = $wpdb->query(
				"ALTER TABLE `{$wpdb->prefix}wc_wallee_transaction_info` ADD COLUMN `user_failure_message` VARCHAR(2047) CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL AFTER failure_reason;"
			);
			if ( false === $result ) {
				throw new Exception( $wpdb->last_error );
			}
		...
		$result = $wpdb->query(
			"CREATE TABLE IF NOT EXISTS {$wpdb->prefix}wc_wallee_attribute_options(
				`id` int(10) unsigned NOT NULL AUTO_INCREMENT,
	...
	public static function update_1_0_6_shorten_table_names() {
		...
		$table_rename_map = array(
			"{$wpdb->prefix}woocommerce_wallee_attribute_options" => "{$wpdb->prefix}wc_wallee_attribute_options",
			"{$wpdb->prefix}woocommerce_wallee_completion_job" => "{$wpdb->prefix}wc_wallee_completion_job",
			"{$wpdb->prefix}woocommerce_wallee_method_configuration" => "{$wpdb->prefix}wc_wallee_method_config",
			"{$wpdb->prefix}woocommerce_wallee_refund_job" => "{$wpdb->prefix}wc_wallee_refund_job",
			"{$wpdb->prefix}woocommerce_wallee_token_info" => "{$wpdb->prefix}wc_wallee_token_info",
			"{$wpdb->prefix}woocommerce_wallee_transaction_info" => "{$wpdb->prefix}wc_wallee_transaction_info",
			"{$wpdb->prefix}woocommerce_wallee_void_job" => "{$wpdb->prefix}wc_wallee_void_job",
		);
